...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE vehicles SET 
                brand = ?, model = ?, usd_price = ?, gear_box = ?, fuel_type = ?, 
                max_speed = ?, capacity = ?, fuel_tank = ?, mileage = ?, 
                main_image = ?, image1 = ?, image2 = ?, image3 = ? WHERE id = ?");
        
        $success = $stmt->execute([
            $_POST['brand'], $_POST['model'], $_POST['usd_price'], $_POST['gear_box'], 
            $_POST['fuel_type'], $_POST['max_speed'], $_POST['capacity'], 
            $_POST['fuel_tank'], $_POST['mileage'], $main_image, $image1, $image2, $image3, $id
        ]);
